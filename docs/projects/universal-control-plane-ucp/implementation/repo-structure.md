...
| `cli` | Go binary (submodule) | Command-line client |
...
separate repositories, plus a submodule pointer update in the umbrella repo.

If an OpenAPI spec is introduced as a shared contract between `api-server` and `cli`,
keeping it in sync across separate repos becomes a manual coordination problem. The
decision is how to reorganize these components to reduce coordination overhead without
sacrificing independent deployment capability.
