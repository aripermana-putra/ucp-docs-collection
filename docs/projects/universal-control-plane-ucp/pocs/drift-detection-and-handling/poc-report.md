...
|---|---|---|
| Scan cost scales with resource count — each scan cycle iterates all drift-protected MRs | Medium | Two-phase workflow: `DiscoverMRsActivity` per GVR + `ScanTenantActivity` per `(GVR, tenant)`. Load distributed across worker pods automatically via Temporal task queue. Dynamically autoscale workers using queue-depth-based autoscalers (e.g. KEDA). |
