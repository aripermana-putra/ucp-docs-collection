...
| FP-03 | No false positive on `bootDisk.initializeParams.image` | GCE image family `debian-cloud/debian-12` is resolved by GCP to a versioned URL. Run watcher against a real GCE instance MR. Confirm no drift on `bootDisk[0].initializeParams[0].image`. | ✅ | ✅ | ✅ | ✅ |
...
| A-04 | Restart after crash — already-drifted resource is re-detected | Kill the watcher while a drift event is in flight. Restart. Verify on the next poll the drifted resource is detected again. | ✅ |
...
|----|---------|--------|--------| ------ |
| D-S01 | Scan schedule fires every minute | `temporal schedule describe --schedule-id drift-scan`. Confirm `RecentActions` shows runs at 1-min intervals. | — | Unable to test because currently temporal internal worker is not running |
| D-S02 | `DriftScanWorkflow` output reports correct counts | After a scan with N resources watched and N drifted, check Temporal UI for the completed `DriftScanWorkflow`. Result should show the same count of N `scannedResources: N, driftedResources: N`. | ✅ |
...
| EC-05 | Field drift and deletion occur simultaneously (both signals fire for same MR) | Externally modify a field AND delete the resource in quick succession. Both Signal 1 and Signal 2 should appear in the same drift report (`changes (2)`). | ✅ | ✅ | ✅ | ✅ |
| EC-06 | Observed value is `nil` vs desired is a non-empty map | A GCP resource configuration removes a field group from `atProvider` that was present in `forProvider` (e.g. optional feature disabled, GCP stops returning the section). Expected: treated as no-diff (no observed baseline = cannot conclude drift). | ✅ | ✅ | ✅ | ✅ |
| EC-07 | Desired slice has elements but observed slice is shorter | `forProvider` declares 3 node locations, GCP only reports 2 in `atProvider`. Expected: the missing index is reported as `<missing>` in the drift output. | ✅ | ✅ | ✅ | ✅ |
